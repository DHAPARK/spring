...
public class MoveController {
	...
	public void getManagerPage() throws Exception {
		logger.info("get managerpage");
	}
	
	//메인화면 post
	@RequestMapping(value = "/managerpage", method = RequestMethod.POST)
	public String postManagerPage(MemberVO vo) throws Exception {
	...
	public void getContact(@ModelAttribute("scri") SearchCriteria scri, Model model, HttpServletRequest req ) throws Exception {
	...
	public void getManagerGoodsList(@ModelAttribute("scri") SearchCriteria scri, Model model, HttpServletRequest req) throws Exception{
		...
		HttpSession session = req.getSession();
		MemberVO member = (MemberVO) session.getAttribute("member");
		List<GoodsVO> list=moveService.goodslistSearch(scri);
		model.addAttribute("member", member);
		model.addAttribute("list", list);
